...
**The Today view is tracking-state-driven.** It renders one of two modes: an idle **Launchpad** (`#td-idle`) or a **Focus** engine (`#td-focus`). `renderTodayView(metrics)` (called from `updateDashboardUI`) chooses via `resolveTrackingMode(current, groups, stats)` and calls either `renderIdle` (→ `renderRunway` / `renderLanes` / `renderMomentum`) or `renderFocus` (a live SVG ring driven by a 1s `updateFocusLive` ticker). The runway/day-timeline math is the pure, tested `computeRunway`.
