...
**Standalone dev fallback.** When `window.PluginAPI` is absent (opening `index.html` directly, or in tests), the code falls back to built-in mock data so the dashboard renders without the host. The standalone bootstrap reads `?scenario=NAME` (`idle` | `cold` | `met` | `empty` | `focus` | `focus-met` | `focus-ungrouped`) to preview each Today state (pair `cold` with a pre/early-first-block wall-clock); `scripts/screenshots.mjs` drives these with a faked wall-clock to produce `screenshots/*.png`.
